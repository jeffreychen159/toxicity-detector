# Cross-Domain Evaluation: MLP Classifier on Real Toxicity Prompts (Reddit)
import os
...
print("="*60)
print("CROSS-DOMAIN EVALUATION: MLP CLASSIFIER ON REDDIT DATA")
print("="*60)
...
if os.path.exists(pth_path):
    try:
        ...
        # We need to load the transformer and model
        # For now, create a new transformer and load model state
        transformer = TfidfSVDTransformer(max_features=20000, n_components=512)
        model = MLP(input_size=512, output_size=6, dropout=0.3)
...
if model_loaded:
    ...
    # For MLP, we need to fit the transformer on the training data first
    # Since we don't have access to the original transformer, we'll create a new one
    # This means the features might not be exactly the same as training, but should be reasonable
    X_reddit = transformer.fit_transform(texts)
    ...
    with torch.no_grad():
        for i in range(0, len(X_reddit_tensor), batch_size):
            batch_X = X_reddit_tensor[i:i+batch_size].to(device)
    ...
    toxic_logits = all_logits[:, 0]  # Shape: (num_samples,)
    toxic_probs = 1 / (1 + np.exp(-toxic_logits))  # Sigmoid
    ...
    model_description = pth_path if os.path.exists(pth_path) else 'unknown'
    ...
    print("\n" + "="*60)
    print("CROSS-DOMAIN RESULTS: MLP ON REDDIT TOXICITY DETECTION")
    print("="*60)
